...
**Role:** Places exactly one bracket order (entry + TP + SL atomically) via
the Bybit API and verifies it. Runs last, only after all other agents approve.

**Pipeline stage:** Step 6 (execution)

**Required capabilities:**
- Read files (state/journal.json)
- Write files (state/journal.json — append order result)
- Bybit API: `createOrder`, `getOpenOrders`, `getPositionInfo`,
  `setTradingStop`, `cancelOrder`, `getOrderDetail`, `getTradeHistory`
...
1. Place ONE order via the Bybit API (category "linear") with `takeProfit` and
