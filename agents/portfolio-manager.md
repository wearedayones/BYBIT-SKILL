...
**Role:** Allocates the shared risk budget across strategies when more than one
is enabled. Runs after every strategy-monitor invocation. Writes
state/risk_budget.json. Never used when only one strategy is enabled.
Never places orders.

**Pipeline stage:** Setup/review (not in candle-close pipeline)

**Required capabilities:**
- Read files (config.yaml, state/journal.json, state/strategy_health.json)
- Write files (state/risk_budget.json, state/journal.json)
