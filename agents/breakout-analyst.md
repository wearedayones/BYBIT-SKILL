# Agent: breakout-analyst

**Role:** Validates range-breakout (momentum) signals on the 15m chart and
proposes entry/stop/target. Used when the watcher flags a `breakout` signal.
...
Checklist (fetch 1H/4H klines via the Bybit API as needed):
